        ...
        <div className="page-header px-7 py-5 shrink-0">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white" style={{ background: 'linear-gradient(135deg, var(--accent) 0%, #a07830 100%)' }}>
                <ClipboardList size={20} />
              </div>
              <div>
                <h1 className="text-[22px] font-semibold tracking-tight">Exámenes y evaluaciones</h1>
                <p className="text-[13px] mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  Forms · autocalificación · certificados · analíticas
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-auto p-7">
          {loading ? (
            ...
            <div className="card p-16 text-center max-w-lg mx-auto">
              ...
              <p className="text-[15px] font-semibold">Sin evaluaciones</p>
              <p className="text-[13px] mt-1 mb-4" style={{ color: 'var(--text-secondary)' }}>
                Crea un formulario con autocalificación para medir competencias.
              </p>
              <button type="button" className="btn btn-primary" onClick={openNewExam}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 animate-in">
              {exams.map((ex) => {
                ...
                return (
                  <div key={ex.id} className="card card-hover p-5 flex flex-col justify-between relative overflow-hidden group">
                    <div className="absolute top-0 left-0 right-0 h-1.5" style={{ background: th.accentColor }} />
                    <div>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <span className={`badge ${ex.estado === 'Activo' ? 'badge-success' : 'badge-muted'}`}>● {ex.estado}</span>
                        <div className="flex gap-0.5 opacity-70 group-hover:opacity-100">
                          <button type="button" className="btn-icon" onClick={() => handleShareLink(ex)} title="Copiar enlace"><Share2 size={14} /></button>
                          <button type="button" className="btn-icon" onClick={() => openEditExam(ex)} title="Editar"><Pencil size={14} /></button>
                          <button type="button" className="btn-icon" style={{ color: 'var(--danger)' }} onClick={() => ex.id && handleDeleteExam(ex.id)} title="Eliminar"><Trash2 size={14} /></button>
                        </div>
                      </div>
                      <h3 className="text-[15px] font-semibold leading-snug line-clamp-2 mb-1">{ex.titulo}</h3>
                      <p className="text-[12px] line-clamp-2 mb-3" style={{ color: 'var(--text-secondary)' }}>
                        {ex.descripcion || ex.tema || 'Evaluación técnica'}
                      </p>
                      <div className="flex flex-wrap gap-3 text-[11px]" style={{ color: 'var(--text-muted)' }}>
                        <span className="inline-flex items-center gap-1"><HelpCircle size={12} /> {ex.preguntas?.length || 0} preg.</span>
                        <span className="inline-flex items-center gap-1"><Clock size={12} /> {ex.config?.tiempoLimiteMinutos || 15} min</span>
                        <span className="inline-flex items-center gap-1"><Award size={12} /> Min. {ex.config?.notaMinimaAprobatoria || 14}/20</span>
                      </div>
                    </div>
                    <div className="pt-3 mt-3 flex gap-2" style={{ borderTop: '1px solid var(--border)' }}>
                      <button type="button" className="btn btn-ghost text-[12px] flex-1 h-8" onClick={() => openAnalytics(ex)}>
                        <BarChart3 size={14} /> Analíticas
                      </button>
                      <button type="button" className="btn btn-primary text-[12px] flex-1 h-8" onClick={() => startTakingExam(ex)}>
                        <Play size={13} /> Rendir
                      </button>
                    </div>
                  </div>
  ...
  if (view === 'builder') {
    return (
      <div className="h-full flex flex-col animate-in">
        <div className="page-header px-7 py-3 flex flex-wrap items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3">
            <button type="button" className="btn-icon" onClick={() => setView('list')}><ArrowLeft size={18} /></button>
            <div>
              <span className="text-[11px] font-bold uppercase tracking-wider" style={{ color: 'var(--accent)' }}>Forms Studio</span>
              <h1 className="text-[17px] font-semibold">{editingExam?.id ? 'Editar evaluación' : 'Nueva evaluación'}</h1>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex gap-1 p-1 rounded-lg" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
              {(['preguntas', 'config', 'theme'] as const).map((tab) => (
                <button
                  key={tab}
                  type="button"
                  onClick={() => setActiveTabBuilder(tab)}
                  className="px-3 py-1.5 rounded-md text-[12px] font-semibold"
                  style={{
                    background: activeTabBuilder === tab ? 'var(--surface)' : 'transparent',
                    color: activeTabBuilder === tab ? 'var(--primary-text)' : 'var(--text-secondary)',
                    boxShadow: activeTabBuilder === tab ? 'var(--shadow-sm)' : 'none',
                  }}
                >
                  {tab === 'preguntas' ? `Preguntas (${preguntas.length})` : tab === 'config' ? 'Opciones' : 'Tema'}
                </button>
              ))}
            </div>
            <button type="button" className="btn btn-primary" onClick={handleSaveExam}><Save size={15} /> Guardar</button>
          </div>
        </div>

        <div className="flex-1 overflow-auto p-7 max-w-3xl mx-auto w-full">
          <div className="card mb-5 overflow-hidden">
            <div className="h-16 px-5 flex items-center justify-between text-white" style={{ background: activeTheme.headerBg }}>
              <div>
                <div className="text-[10px] font-bold uppercase tracking-widest opacity-80">EXPORTADORA ROMEX S.A.</div>
                <div className="text-[13px] font-semibold">Evaluación de competencias</div>
              </div>
            </div>
            <div className="p-5 space-y-3">
              <input className="input w-full text-[18px] font-semibold h-11" value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Título del examen…" />
              <textarea className="input w-full" rows={2} value={descripcion} onChange={(e) => setDescripcion(e.target.value)} placeholder="Instrucciones para el evaluado…" />
              <div className="grid sm:grid-cols-2 gap-3">
                <select className="input w-full" value={temaId === '' ? '' : String(temaId)} onChange={(e) => setTemaId(e.target.value ? Number(e.target.value) : '')}>
                  <option value="">— Sin vincular —</option>
                  {temas.map((t) => <option key={t.id} value={t.id}>{t.tema}</option>)}
                </select>
                <select className="input w-full" value={estado} onChange={(e) => setEstado(e.target.value as typeof estado)}>
            ...
            <div className="space-y-3">
              {preguntas.map((q, idx) => (
                <div key={q.id} className="card p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-3 pb-2" style={{ borderBottom: '1px solid var(--border)' }}>
                    <div className="flex items-center gap-2">
                      <span className="w-6 h-6 rounded-full text-[12px] font-bold flex items-center justify-center" style={{ background: 'var(--primary-soft)', color: 'var(--primary-text)' }}>{idx + 1}</span>
                    ...
                    <div className="flex items-center gap-1">
                      <input type="number" min={1} max={20} className="input h-8 w-14 text-center text-[12px]" value={q.puntos} onChange={(e) => updateQuestion(q.id, { puntos: Number(e.target.value) || 1 })} />
                  ...
                  <input className="input w-full mb-3" value={q.texto} onChange={(e) => updateQuestion(q.id, { texto: e.target.value })} placeholder="Enunciado…" />
                  {(q.tipo === 'multiple' || q.tipo === 'casillas' || q.tipo === 'desplegable') && (
                    <div className="space-y-2">
                      {(q.opciones || []).map((op, opIdx) => {
                        ...
                        return (
                          <div key={opIdx} className="flex items-center gap-2">
                            ...
                            <input className="input flex-1 h-8 text-[13px]" value={op} onChange={(e) => {
                      ...
                      <button type="button" className="text-[12px] font-semibold" style={{ color: 'var(--primary-text)' }} onClick={() => updateQuestion(q.id, { opciones: [...(q.opciones || []), `Opción ${(q.opciones?.length || 0) + 1}`] })}>+ Añadir opción</button>
                    </div>
                  )}
                  {q.tipo === 'verdadero_falso' && (
                    <div className="flex gap-2">
                      {['Verdadero', 'Falso'].map((vf, i) => (
                        <button key={vf} type="button" className="flex-1 py-2 rounded-lg text-[13px] font-semibold border" style={{
                          background: Number(q.correcta) === i ? 'var(--success-soft)' : 'var(--surface-2)',
                          color: Number(q.correcta) === i ? 'var(--success)' : 'var(--text-secondary)',
                          borderColor: Number(q.correcta) === i ? 'var(--success)' : 'transparent',
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <input className="input flex-1 h-8 text-[11px] border-0 bg-transparent" placeholder="Explicación al revisar…" value={q.explicacion || ''} onChange={(e) => updateQuestion(q.id, { explicacion: e.target.value })} />
                  </div>
                </div>
              ))}
              <div className="card p-3 flex flex-wrap gap-2 justify-center border-dashed">
                {QUESTION_TYPES.map((t) => (
                  <button key={t.type} type="button" className="btn btn-ghost text-[12px] h-8" onClick={() => addQuestion(t.type)}>{t.icon} {t.label}</button>
                ))}
              </div>
            </div>
          )}
          ...
          {activeTabBuilder === 'config' && (
            <div className="card p-5 space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <label className="block text-[12px] font-semibold">Tiempo (min)
                ...
                <label className="block text-[12px] font-semibold">Nota mínima /20
          ...
          {activeTabBuilder === 'theme' && (
            <div className="card p-5">
              <h3 className="text-[15px] font-semibold mb-1 flex items-center gap-2"><Palette size={16} /> Tema visual</h3>
              <p className="text-[12px] mb-4" style={{ color: 'var(--text-secondary)' }}>Fondo que verá el evaluado al rendir.</p>
              <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                {FORM_THEMES.map((theme) => (
                  <button
                    ...
                    className="rounded-xl p-4 text-left text-white h-28 transition-transform hover:scale-[1.02]"
                    style={{
                      background: theme.bgStyle,
                      boxShadow: selectedThemeId === theme.id ? '0 0 0 3px var(--primary)' : 'none',
                    }}
                  >
                    <div className="font-bold text-[12px]">{theme.name}</div>
                    {selectedThemeId === theme.id && <div className="mt-2 text-[11px] font-bold opacity-90">✓ Seleccionado</div>}
                  </button>
                ))}
              </div>
            </div>
