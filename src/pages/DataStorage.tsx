    ...
    <div className="h-full flex min-h-0" style={{ background: 'var(--bg)' }}>
      <aside className="w-[248px] shrink-0 flex flex-col border-r overflow-y-auto" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
        ...
        <div className="mt-4 px-2 flex-1 min-h-0 flex flex-col">
          <button type="button" className="w-full flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }} onClick={() => setYearsOpen((o) => !o)}>
            {yearsOpen ? <ChevronDown size={12} /> : <ChevronRight size={12} />} Temas · {year}
          </button>
          {yearsOpen && (
            <div className="mt-1 space-y-0.5 overflow-y-auto flex-1">
              <div className="flex gap-1 px-2 mb-1">
                {[2025, 2026, 2027].map((y) => (
                  <button key={y} type="button" onClick={() => { setYear(y); setSelectedFolder(null); setSection('all') }} className="flex-1 text-[11px] font-semibold py-1 rounded-md" style={{ background: year === y ? 'var(--primary-soft)' : 'var(--surface-2)', color: year === y ? 'var(--primary-text)' : 'var(--text-secondary)' }}>{y}</button>
                ))}
              </div>
              {folders.map((f) => (
                <button key={f.id} type="button" onClick={() => openFolder(f)} className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-left text-[12px]" style={{ background: selectedFolder?.id === f.id ? 'var(--primary-soft)' : 'transparent', color: selectedFolder?.id === f.id ? 'var(--primary-text)' : 'var(--text-secondary)', fontWeight: selectedFolder?.id === f.id ? 600 : 400 }} title={f.tema}>
                  <FolderIcon /><span className="truncate">{f.tema}</span>
                </button>
              ))}
              {!folders.length && <p className="px-3 py-2 text-[11px]" style={{ color: 'var(--text-muted)' }}>Sin carpetas. Abre Cronograma.</p>}
            </div>
          )}
        </div>
            ...
            <div className="table-wrap">
              ...
              {!filteredFolders.length && !filteredFiles.length && (
                <div className="p-14 text-center">
                  ...
                  <p className="text-[12px] mt-1 mb-4" style={{ color: 'var(--text-secondary)' }}>{selectedFolder ? 'Sube materiales de esta capacitación' : 'Selecciona un tema a la izquierda'}</p>
                  {selectedFolder && (
