...
import {
  ...
  FileDown, LayoutTemplate, Settings, Moon, Sun,
} from 'lucide-react'
...
type View = 'vault' | 'graph' | 'cronograma' | 'files' | 'examenes' | 'auditoria' | 'config'
...
const NAV: { id: View; icon: typeof FileText; label: string }[] = [
  ...
  { id: 'examenes', icon: CheckSquare, label: 'Examenes' },
  { id: 'auditoria', icon: CheckSquare, label: 'Auditoria' },
  { id: 'config', icon: Settings, label: 'Configuracion' },
]

function Chip({ children, tone = 'blue' }: { children: React.ReactNode; tone?: string }) {
  ...
  return <span className={'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ' + (tones[tone] || tones.blue)}>{children}</span>
}
...
export default function App() {
  const [theme, setTheme] = useState<'light' | 'dark'>(() => {
    try {
      const s = localStorage.getItem('cap-theme')
  ...
  function toggleTheme() {
    ...
    setTheme(next)
    try { localStorage.setItem('cap-theme', next) } catch {}
  ...
  function exportAudit() {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'Capacitaciones-auditoria-' + year + '.json'; a.click()
  }
  ...
  if (!ready) return <div className="h-full flex items-center justify-center bg-[#f8f9fa] text-[#5f6368]">Cargando Capacitaciones...</div>
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="h-14 bg-white border-b border-[#e8eaed] flex items-center px-3 gap-3 shadow-sm z-10 shrink-0">
        ...
        <span className="text-[22px] text-[#1a73e8] font-medium">Capacitaciones</span>
        ...
        <button type="button" onClick={toggleTheme} title="Cambiar tema" className="w-10 h-10 rounded-full flex items-center justify-center text-[#5f6368] hover:bg-[#f1f3f4]">
      ...
      <div className="flex-1 flex min-h-0">
        <nav className={'bg-white border-r border-[#e8eaed] flex flex-col items-center py-3 gap-1 shrink-0 transition-all duration-300 overflow-hidden ' + (navOpen ? 'w-16' : 'w-0 border-0')}>
          {NAV.map(({ id, icon: Icon, label }) => {
            ...
            const cls = active
              ? 'w-12 h-12 rounded-2xl flex items-center justify-center bg-[#e8f0fe] text-[#1a73e8]'
              : 'w-12 h-12 rounded-2xl flex items-center justify-center text-[#5f6368] hover:bg-[#f1f3f4]'
            return (
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-[#f8f9fa]">
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-10">
                <article className="max-w-[720px] mx-auto bg-white rounded-xl shadow-sm border border-[#e8eaed] p-10">
                  <h1 className="text-[28px] font-normal mb-2">{currentNote.title}</h1>
                  ...
                  <div className="text-[15px] leading-relaxed" dangerouslySetInnerHTML={{ __html: renderMarkdown(currentNote.body) }}
            ...
            <div className="flex-1 min-h-0 p-4">
              <GraphView
                ...
                onOpenNote={(id) => {
                  ...
                  if (n) { setCurrentNoteId(n.id); setView('vault') }
                }}
              />
            </div>
          )}
                ...
                {events.length === 0 ? <div className="p-12 text-center text-[#80868b] text-sm">Sin capacitaciones</div> :
                  [...events].sort((a, b) => a.date.localeCompare(b.date)).map(e => (
                    <div key={e.id} className="flex items-center gap-3 px-5 py-3.5 border-b border-[#e8eaed] last:border-0 hover:bg-[#f8f9fa]">
                      ...
                      <button type="button" onClick={async () => { if (confirm('Eliminar?')) { await db.deleteEvent(e.id); await refresh() } }} className="p-1.5 rounded-full hover:bg-[#fce8e6]"><Trash2 size={16} /></button>
                    </div>
                  ))}
              </div>
            </div>
          ...
          {view === 'config' && (
            <SettingsPanel theme={theme} onToggleTheme={toggleTheme} notes={notes} events={events} files={files} exams={exams} />
          )}
        </main>
      ...
      {previewFile && (
        <FilePreview file={previewFile} onClose={() => setPreviewFile(null)} onUpdated={async () => { await refresh() }} />
      )}
      ...
      <Modal open={eventModal} onClose={() => setEventModal(false)} title="Nueva capacitacion">
        ...
        <label className="block mb-4"><span className="block text-xs font-medium text-[#5f6368] mb-1.5">Fecha</span><input type="date" className={inputCls} value={form.date || ''} onChange={e => setForm({ ...form, date: e.target.value })} /></label>
        <label className="block mb-4"><span className="block text-xs font-medium text-[#5f6368] mb-1.5">Hora</span><input type="time" className={inputCls} value={form.time || '09:00'} onChange={e => setForm({ ...form, time: e.target.value })} /></label>
